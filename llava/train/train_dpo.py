...
class TrainingArguments(transformers.TrainingArguments):
    ...
    draw_gradcam: bool = field(default=False)
...
class DPODataset(Dataset):
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image_rejected" in sources[0]:
            image_rejected_file = self.list_data_dict[i]["image_rejected"]
            if type(image_rejected_file) is list:
                raise NotImplementedError("Multiple rejected images are not supported yet.")
            else:
                image_rejected = [self.process_image(image_rejected_file)]
            # sources = preprocess_multimodal(copy.deepcopy([e["conversations"] for e in sources]), self.data_args)

        has_image = ("image" in self.list_data_dict[i]) or ("video" in self.list_data_dict[i])
        has_image_rejected = ("image_rejected" in self.list_data_dict[i]) 
        ...
        if "image_rejected" in self.list_data_dict[i]:
            data_dict["image_rejected"] = image_rejected
        
        # prompt exist in the data
        data_dict["has_image"] = has_image
        data_dict["has_image_rejected"] = has_image_rejected
        return data_dict
...
class DPODataCollator(DPODataCollatorWithPadding):
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for feature in features:
            ...
            has_image_rejected_list.append(bool(feature["has_image_rejected"]))

        # return collated batch
        padded_batch = self.collate(tokenized_batch)
        padded_batch["has_image_rejected"] = has_image_rejected_list
        # import pdb;pdb.set_trace()
        if "image" in features[0]:
            ...
            _imgrej_by_feat = []
            for f in features:
                if "image_rejected" in f:
                    _imgrej_by_feat.append(f["image_rejected"])  # [(tensor, size, modality)]
                else:
                    _imgrej_by_feat.append(f["image"])   

            padded_batch["image_rejected_sizes"]      = [im[1] for im_list in _imgrej_by_feat for im in im_list]
            padded_batch["image_rejected_modalities"] = [im[2] for im_list in _imgrej_by_feat for im in im_list]
            padded_batch["images_rejected"]           = [im[0] for im_list in _imgrej_by_feat for im in im_list]
...
def train(attn_implementation=None):
    ...
    if training_args.draw_gradcam:
        from llava.train.hook import unwraped_and_tapping
        trainer = unwraped_and_tapping(trainer, training_args)
