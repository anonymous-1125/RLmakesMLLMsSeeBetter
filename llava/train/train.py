...
class LazySupervisedDataset(Dataset):
    ...
    def fix_image_format_error(self, image_file, image_folder):
        import cv2
        ...
        try:
            ...
            print(f"[INFO] Fixed and overwrote image: {image_file}")
            return image_file

        except Exception as e:
            print(f"[ERROR] OpenCV conversion failed for {image_file}: {e}")
            raise e
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        try:
            ...
            converted_file = self.fix_image_format_error(image_file, image_folder)
            image_path = os.path.join(image_folder, converted_file)  
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        return data_dict
        # len(data_dict['image'][0]) = 3
        # data_dict['image'][0][0] = [num_resized_image, 3, height, width] # one image is resized by squared, croped and padded.
        # data_dict['image'][0][1] = (height, width) # original image size
